...
5. **Human is the review gate, not you.** The human does not read diffs himself; coding agents review. High-risk task closure requires a **human acceptor** — you cannot close those; you move them to `review` and stop. (See §5.)
...
- **OPEN-2 — should these three `review` tasks be human-accepted?** `OPS-T-0007/0008` and `RUN-T-0043` sit in `review` awaiting the human's **batch acceptance in the serve UI**. High-risk close requires a human acceptor — **you cannot and must not close them** (a direct `tusker close` on a high-risk task returns `[INVALID_TRANSITION] close requires human acceptor for high risk`). This is the human's gate, by design. Leave them until he accepts.
...
- **`[INVALID_TRANSITION] status cannot set done directly`** → use `tusker close`. And **`close requires human acceptor for high risk`** is *not* a bug to route around — it's the human's gate (OPEN-2). Move to `review` and stop.
