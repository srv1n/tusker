...
description: "Runtime state, events, and generated caches."
tusker:
  agent_layer: "capsule"
  audience: "developer"
  ...
  mode: "reference"
  publish_path: "internal/reference/runtime"
  route: "/internal/reference/runtime/"
  source_kind: "vault_doc"
  source_of_truth:
    - "cmd/tusker/daemon.go"
    - "cmd/tusker/runtime_store.go"
  source_path: "docs/reference/runtime.md"
  stale_when_paths:
    - "cmd/tusker/daemon.go"
    - "cmd/tusker/runtime_store.go"
    - "skill/docs/ORCHESTRATION_RUNBOOK.md"
  summary: "Runtime state, events, and generated caches."
  tags: []
  updated: "2026-05-04"
---
...
Tusker V5 keeps task truth in markdown and runtime truth outside markdown. The Codex-only orchestration surface is local and operator-facing: register a repo/vault, run one daemon tick or the long-lived daemon, and inspect runtime evidence without putting leases or transcripts into task frontmatter.

## State boundary

| State | Location | Examples |
|---|---|---|
| Task truth | task markdown frontmatter/body | `status`, `assignee`, `blocked_by`, `block_reason`, evidence, verification, work log |
...
stateDiagram-v2
  ...
  running --> review: Codex changes task to review, then packet is written
  review --> done: human verify + close
  review --> rework: human requests follow-up
  active --> blocked: Codex or human records blocker
  blocked --> ready: blocker resolved
```

The daemon does not blindly move active tasks to review. The runner prompt asks Codex to move the task to `review` when ready. If the task is still `active` or `rework` after a clean exit, the daemon queues continuation. If Codex moves the task to `review` and exits cleanly, the daemon writes the review packet and releases the run as succeeded.
...
| run | current lease state, attempt outcome, active attempt, workspace, session, pid, artifact paths, work revision, retry/error timestamps |
| attempt | immutable-ish execution attempt snapshot with outcome, exit code, artifact paths, pid, start/finish |
| turn | normalized Codex turn status, session, token usage, last event/error |
...
When a clean run reaches a non-active task state, the daemon can write a review packet under `Attachments/<TASK-ID>/review-packet-<attempt>.md` and append packet evidence to the task. The packet is evidence, not approval: it summarizes daemon-observed facts and still requires human verification before close.
