...
At tier 1 (`tier: 1` in `.tusker/config.yaml`) create/status/close work as a plain tracker: no contract check on ready, no proof gate on close. Record what exists; ceremony arrives with higher tiers, not before.
