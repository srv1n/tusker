...
test("task capsules expose open gate identity and action in both Work views", () => {
  const board = read("src/features/work/WorkBoard.tsx");
  const table = read("src/features/work/WorkTable.tsx");
  const parts = read("src/features/work/WorkParts.tsx");

  expect(board).toContain("task.openGates");
  expect(board).toContain("<GateSummary");
  expect(table).toContain("task.openGates");
  expect(table).toContain("<GateSummary");
  expect(parts).toContain("gate.id");
  expect(parts).toContain("gate.action");
});
...
test("needs consume explicit gate IDs and compact triage keys actions by gate", () => {
  const needs = read("src/components/needs/NeedCard.tsx");
  const panel = read("src/features/panel/Panel.tsx");
  expect(needs).toContain("need.gateId ??");
