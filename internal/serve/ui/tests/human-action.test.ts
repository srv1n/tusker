...
test("human action card renders the served contract and one contextual completion path", () => {
  ...
  expect(card).toContain("Approve and continue");
  expect(card).not.toContain("Mark complete");
  expect(card).toContain("Return to rework");
  expect(card).toContain("blocks ");
  expect(card).toContain("action.gateId");
  expect(card).toContain("gateAction.mutateAsync");
  expect(card).toContain("server-authorized action");
  expect(card).not.toContain("requestHumanReceipt");
