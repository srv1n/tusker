...
function TaskRow({
  ...
  return (
    <Link
      ...
      className={cn(
        GRID,
        "group rounded-md border-b border-line-soft px-2.5 py-2 transition-colors last:border-b-0 hover:bg-hover focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-accent/30",
      )}
    >
...
function EpicHeader({
  ...
  return (
    <div className="mb-1 flex items-center gap-2.5 border-b border-line px-1 pb-2 pt-6 first:pt-1">
  ...
  return (
    <div className="tk-scroll animate-rise overflow-x-auto">
      <div className="min-w-[840px]">
        {/* Sortable header */}
        <div className={cn(GRID, "border-b border-line px-2.5 pb-2")}>
          {headers.map((header) => {
            ...
            ? buckets.map((b) => {
                ...
                return (
                  <Fragment key={b.epicId}>
                    ...
                    />
                    {b.rows.map((r) => (
                      <TaskRow key={r.original.id} task={r.original} projectId={projectId} grouped />
                    ))}
                  </Fragment>
                );
              })
            : rows.map((r) => (
                <TaskRow key={r.original.id} task={r.original} projectId={projectId} grouped={false} />
              ))}
        </div>
      </div>
    </div>
