...
/**
 * The v2 project settings surface deliberately exposes only the narrow write
    ...
    <div className="space-y-9">
      <SettingGroup label="Automation">
        <SettingRow
          ...
          detail="Allows the resident daemon to pick up authorized work for this project."
...
function RuntimeTab({ daemon }: { daemon: DaemonStatus }) {
  ...
  ]} /><UnavailablePanel title="Runtime controls" detail="Start, stop, and resume exist as low-level daemon mutations, but the v2 runtime panel does not expose a restart or scheduling-pause control because those exact operations are not provided by the Serve contract." /></div>;
}
