      ...
      <main className="mx-auto w-full max-w-6xl" data-delivery-review-page>
        <header className="mb-6 flex flex-col gap-4 border-b border-line pb-5 sm:flex-row sm:items-end sm:justify-between">
          ...
          <form onSubmit={(event) => { event.preventDefault(); refresh(); }} className="flex w-full gap-2 sm:w-auto" aria-label="Delivery plan">
            <input aria-label="Repo-relative plan path" value={plan} onChange={(event) => setPlan(event.target.value)} placeholder="docs/plans/example-v2.yaml" autoComplete="off" spellCheck={false} className="min-w-0 flex-1 rounded-lg border border-line bg-panel px-3 py-2 font-mono text-xs text-ink outline-none focus:border-accent sm:w-80" />
            <button type="submit" disabled={!plan.trim()} className="inline-flex items-center gap-1.5 rounded-lg border border-line px-3 py-2 text-sm font-medium text-ink hover:bg-hover disabled:cursor-not-allowed disabled:opacity-45"><RefreshCw size={14} />Review</button>
          </form>
        </header>

        {!plan.trim() && <StateCard tone="info" title="Choose a delivery plan" detail="Enter or paste a repository-relative V2 plan path, then choose Review." />}
        ...
        {inputMatchesReview && data && <ReviewBody review={data} confirmation={confirmation} setConfirmation={setConfirmation} exactConfirmation={exactConfirmation} startResult={mutationMatchesReview ? start.data : undefined} startError={mutationMatchesReview ? start.error : null} starting={mutationMatchesReview && start.isPending} onStart={() => start.mutate({ plan: submittedPlan, confirm: confirmation.trim(), planIdentity: reviewedIdentity ?? "" })} />}
      </main>
    </PageScroll>
  );
}
