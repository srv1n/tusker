...
function DeliveryPlanCard({ plan, selected, onChoose }: { plan: DeliveryPlanSummary; selected: boolean; onChoose: () => void }) {
  ...
  return <button type="button" onClick={onChoose} disabled={!available} className={cn("rounded-[3px] border p-4 text-left transition", selected ? "border-accent bg-accent/5" : "border-line bg-panel hover:border-ink-soft", !available && "cursor-not-allowed opacity-60")}>
    <div className="flex items-start justify-between gap-3"><div><p className="text-sm font-semibold text-ink">{plan.title}</p><p className="mt-1 text-xs leading-5 text-muted">{plan.summary || `${plan.taskCount} task${plan.taskCount === 1 ? "" : "s"} prepared for review.`}</p></div><span className={cn("shrink-0 rounded-full px-2 py-1 font-mono text-[10px]", available ? "bg-info-soft text-info" : "bg-fail-soft text-fail")}>{available ? "Review" : "Needs repair"}</span></div>
    <div className="mt-3 flex flex-wrap gap-1.5 text-[11px] text-faint"><span className="rounded border border-line px-1.5 py-0.5">{plan.taskCount} task{plan.taskCount === 1 ? "" : "s"}</span><span className="rounded border border-line px-1.5 py-0.5">up to {plan.expectedConcurrency} at once</span>{plan.runnerProfile && <span className="rounded border border-line px-1.5 py-0.5">{plan.runnerProfile}</span>}</div>
    {available && <ul className="mt-3 space-y-1 border-t border-line pt-3 text-xs text-muted">{plan.tasks.slice(0, 3).map((task) => <li key={task.sourceKey}><span className="font-medium text-ink-soft">{task.title}</span>{task.runnerProfile && <span className="text-faint"> · {task.runnerProfile}</span>}</li>)}{plan.tasks.length > 3 && <li className="text-faint">+ {plan.tasks.length - 3} more</li>}</ul>}
    {!available && <p className="mt-3 border-t border-line pt-3 text-xs leading-5 text-fail">{plan.issue || "This plan needs repair before review."}</p>}
  </button>;
}
...
function ReviewBody({ review, confirmation, setConfirmation, startResult, startError, starting, onStart }: { review: DeliveryReview; confirmation: string; setConfirmation: (value: string) => void; startResult?: DeliveryStartResult; startError: unknown; starting: boolean; onStart: () => void }) {
  ...
  return <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_22rem]">
    ...
    <aside className="h-fit rounded-[3px] border border-line bg-panel p-4 xl:sticky xl:top-4">
      ...
      {(start.blockers ?? []).length > 0 && <div className="mt-4 rounded-[3px] bg-warn-soft p-3"><p className="text-sm font-semibold text-warn">Blocked</p><ul className="mt-1 list-disc pl-4 text-xs leading-5 text-warn">{(start.blockers ?? []).map((blocker) => <li key={blocker}>{blocker}</li>)}</ul></div>}
      {review.ready && start.state === "held" && <div className="mt-5 border-t border-line pt-4"><label className="block text-xs leading-5 text-muted"><strong className="text-ink">Confirm the exact reviewed plan.</strong><span className="mt-1 block">Paste the fingerprint below. Tusker re-checks the plan, context, routing, and project opt-in before anything is armed.</span><input aria-label="Exact plan fingerprint confirmation" value={confirmation} onChange={(event) => setConfirmation(event.target.value)} placeholder={reviewedFingerprint} spellCheck={false} className="mt-3 w-full border border-line bg-surface px-2 py-2 font-mono text-[10px] text-ink outline-none focus:border-accent" /></label><button type="button" onClick={onStart} disabled={!canStart} className={cn("mt-3 flex w-full items-center justify-center gap-2 rounded-[3px] px-3 py-2.5 text-sm font-semibold", canStart ? "bg-ink text-surface hover:opacity-90" : "cursor-not-allowed bg-hover text-faint")}><Play size={14} />{starting ? "Starting delivery…" : "Start delivery"}</button><p className="mt-2 text-[11px] leading-4 text-faint">This cannot enable automation, start a daemon, satisfy a gate, authorize a release, or approve spend.</p><details className="mt-3 text-[11px] text-faint"><summary className="cursor-pointer">Technical receipt</summary><p className="mt-2 break-all font-mono">Plan version: {start.planFingerprint}</p><p className="mt-1 break-all font-mono">Planning context: {start.contextFingerprint || "Not recorded"}</p></details></div>}
