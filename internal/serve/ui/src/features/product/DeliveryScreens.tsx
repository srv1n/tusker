...
export function Waves() {
  ...
  return (
    <ProductPage title="Waves" eyebrow="Deliveries" intro="Active and completed delivery boundaries, ordered by what is moving or needs attention." wide>
          ...
          {rows.map(({ wave, tasks }) => {
            ...
            return (
              <ProductRow
                ...
                status={<ProductStatus tone={phaseTone(phase)}>{phase === "building" ? "Building" : phase === "checking" ? "Checking" : phase === "blocked" ? "Needs attention" : phase}</ProductStatus>}
...
export function WaveDetail({ waveId: requestedWaveId }: { waveId?: string } = {}) {
  ...
  return (
    <ProductPage title={wave.title} eyebrow={`Wave ${wave.id}`} intro={wave.brief.outcome.summary || summary} wide>
        ...
        {wave.brief.seeIt.length === 0 ? <ProductEmpty title="No accepted artifacts yet" detail="Accepted proof appears here as tasks complete their review." /> : (
          <div className="border-t border-line">
            {wave.brief.seeIt.map((artifact) => (
              <ProductRow key={`${artifact.taskId}-${artifact.evidenceRef}`} meta={artifact.kind} title={artifact.summary} detail={artifact.acceptanceIds.length ? `Covers ${artifact.acceptanceIds.join(", ")}` : "Accepted evidence"} status={<ProductStatus tone="pass">Verified</ProductStatus>} action={<FileCheck2 size={16} className="text-pass" />} />
            ))}
          </div>
        )}
      </ProductSection>
