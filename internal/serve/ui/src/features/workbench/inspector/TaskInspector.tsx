      ...
      >
        ...
        <div className="wux-inspector-body">
          <div className="flex flex-wrap items-center gap-2">
            ...
            {reviewMember?.recovery?.action === "adopt_completed" ? <button type="button" className="wux-inspector-action wux-inspector-action-primary ml-auto" disabled={recovery.isPending || !reviewMember.recovery.enabled} aria-busy={recovery.isPending} onClick={() => recovery.mutate("adopt_completed")} title={reviewMember.recovery.enabled ? "Verify the work already in this project and record it against this task." : reviewMember.recovery.reason}>{recovery.isPending ? "Reconciling…" : "Reconcile completed work"}</button> : null}
            {reviewMember?.recovery?.action === "recover_unknown" ? <button type="button" className="wux-inspector-action wux-inspector-action-primary ml-auto" disabled={recovery.isPending || !reviewMember.recovery.enabled} aria-busy={recovery.isPending} onClick={() => recovery.mutate("recover_unknown")} title={reviewMember.recovery.enabled ? "Inspect and preserve any existing work, verify it, then continue what remains." : reviewMember.recovery.reason}>{recovery.isPending ? "Verifying…" : RECOVERY_ACTION_LABEL}</button> : null}
			{showStart ? <button type="button" className="wux-inspector-action wux-inspector-action-primary ml-auto inline-flex items-center gap-1.5" disabled={!runnable || taskStart.isPending || directiveQueued} onClick={() => taskStart.mutate()} aria-label={`Run task ${task.id}`} title={runBlocker ?? "Queue this task"}><Play size={13} aria-hidden="true" />{directiveQueued ? "Queued" : taskStart.isPending ? "Queuing…" : "Run task"}</button> : null}
          </div>
