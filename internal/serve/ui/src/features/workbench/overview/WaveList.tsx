...
function tag(wave: WaveListItem): string {
  ...
  if (wave.authorization === "armed") return wave.recovery?.queued ? "Armed + Queued" : "Armed on record";
...
/**
 * Shared recovery cause for one row. Authorization, actual activity, and one
 * leading reason come from the server diagnosis; a queued directive is
 * reported as queued, never as running.
 */
function recoveryLine(wave: WaveListItem, backgroundWorkEnabled?: boolean): string | null {
  const recovery = wave.recovery;
  if (!recovery?.blockingCause) return null;
  // The dedicated project-off line below already covers this cause.
  if (backgroundWorkEnabled === false && recovery.causeCode === "project_disabled") return null;
  return recovery.blockingCause;
}
        ...
        <ul className="wux-ov-rows">{rows.map((wave) => <li key={wave.id} className="wux-ov-row">
          <button type="button" className="wux-ov-card" onClick={() => onOpenWave(wave.id)} aria-label={`Open wave ${wave.title}`}>
            <div className="wux-ov-main">
              ...
              {!past && recoveryLine(wave, backgroundWorkEnabled) ? <p className="wux-ov-detail">{recoveryLine(wave, backgroundWorkEnabled)}</p> : null}
            </div>
