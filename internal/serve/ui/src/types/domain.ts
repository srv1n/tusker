...
export interface RecoveryResult {
  ...
  refused?: boolean;
  admitted?: boolean;
  action: "retry_review" | "recover_unknown" | "rerun_checks" | "adopt_completed";
  taskId: string;
  lane?: Lane | "verify";
  reason: string;
}
...
export interface WaveReviewMemberRecovery { action: "retry_task" | "recover_unknown" | "rerun_checks" | "retry_review" | "adopt_completed"; enabled: boolean; reason?: string; attempts?: number; maxAttempts?: number }
export interface WaveReviewMember { taskId: string; title: string; state: string; phase?: "executing" | "awaiting_review" | "reviewing" | "proof_blocked" | "blocked" | "failed" | "outcome_unknown" | "completed" | "paused" | "capacity_wait" | "rework" | "queued"; lane?: Lane; responsible?: string; recovery?: WaveReviewMemberRecovery; completionReported?: boolean; waitingReason?: string; dependencies?: string[]; executeRoute?: string; reviewRoute?: string; acceptance?: string[]; verification?: string[]; instructions?: string; proofInvalidation?: ProofInvalidation }
export interface WaveReviewHumanAction { taskId: string; taskTitle: string; action: HumanAction }
export interface WaveExternalDependency {
  taskId: string;
  title?: string;
  status?: string;
  readiness?: string;
...
export interface WaveReview { schema: "tusker.wave-review/v1"; waveId: string; title: string; outcome: string; state: "Planned" | "Running" | "Paused" | "Waiting" | "Completed" | "Cancelled"; authorization: "inert" | "authorized" | "paused" | "stale"; materialFingerprint: string; members: WaveReviewMember[]; externalDependencies: WaveExternalDependency[]; frontiers: string[][]; humanActions?: WaveReviewHumanAction[]; blockers: DirectStartBlocker[]; controls: DirectStartControl[] }
