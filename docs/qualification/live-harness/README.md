...
| G6 | Muse has no MCP overlay (`runner_muse.go`), so its worker must use `tusker message ask` from the shell. Whether `tusker` is on the Muse worker's `PATH` has not been verified. | The Muse task tells the worker to use the CLI. A `command not found` result is a finding, not a pass. |
