...
5. Check `tusker projects list --json` for stale registrations first (observed: a broken registration made cold start fatal — RUN-T-0021 — disable it or fix it before starting).
...
2026-07-07 observed: fatal exit on a stale project registration (→ RUN-T-0021); after disabling it, boot clean; the sentinel opened the circuit on seven legacy over-cap records — correct honesty; repair needed sqlite surgery because no `runs retire` exists (→ RUN-T-0022); `/api/runs` displayed 15 unclaimed placeholder rows as "running" (→ SRV-T-0013).
...
1. Cancel the probes (`tusker status <probe> cancelled`) and retire their run rows.
