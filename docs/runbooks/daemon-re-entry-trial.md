...
## Trial run #3 verdict (2026-07-07, after the RUN-T-0028/0029/0030 wave)

All seven steps executed; **every hard invariant held**. No dispatch past cap, no continuation loop, no fatal boot, no lease leak, no runaway spend.

**PASSED live:** cold start with a deliberately broken registration quarantined by the shared loader (`health: error`, `WORKFLOW.md not found` recorded, healthy project served) — RUN-T-0029 on `daemon run`; budget probe terminal `budget_exceeded` on attempt 1; poison probe parked `parked_no_progress` at exactly attempt 3 (the cap) with the circuit never opening — RUN-T-0028's pre-dispatch guard proven, and the plan blocker names it honestly (`attempt cap reached (3): dispatch would create another attempt`); redrive reset the window (attempt 3→fresh cap, RUN-T-0024 semantics); real multi-turn attempts ran minutes-stable with `early_exit` correctly recorded on a mid-work exit 0 — RUN-T-0030 proven, ~15s churn gone; kill-9 with both adoption branches correct (surviving wrapper adopted: same pid/generation/attempt; independently-dead wrapper fenced and re-dispatched within cap); `daemon resume` with the broken registration enabled did NOT fatal, verified the repair, closed the circuit — RUN-T-0029 on the resume entry point; drain stop bounded and honest with the wrapper left alive for adoption.

**Soft deviations (filed, non-gating):** RUN-T-0033 — review-flip under a live run opens the global circuit (sentinel's 3s tick always beats the reconciler's 30s poll), so a routine operator action costs a manual resume; lease release took ~101s vs the one-heartbeat target; convergence itself was honest (no re-dispatch, sibling untouched, reconciler still worked under the open circuit). RUN-T-0034 — audit: a wrapper outliving the daemon recorded `succeeded` on a child exit 0 while the task was still `ready`; RUN-T-0030's classification may be missing on the wrapper's direct-store write path.

**Judgment:** the incident classes this trial gates on — runaway attempts, token burn, ghost runs, silent loss — are all structurally dead, proven three runs in a row with the failure modes converging to zero. The two remaining items are operability polish and evidence hygiene, not safety. Recommended: authorize the daemon as executor at cap 5, with RUN-T-0033/0034 as ordinary follow-up tasks.
