...
Manual only. Pin `s1` to a read-only global profile with `tusker task update <s1> --execute-profile <name>`. Pass when Serve shows **Blocked** with a typed permission or sandbox reason. Re-pin a writable profile (or `--clear-execute-profile`), click **Continue**, and verify the task resumes without losing the prior transcript. Record the exact profile and reason.
