...
- **D2 — Runners survive daemon death.** Runners are spawned through the Tusker runner-wrapper in their own process group, stdio redirected to files, no TTY inheritance, stdin from `/dev/null` unless the adapter explicitly wires a pipe (and an adapter that needs a handshake must fail fast if it doesn't complete). They write events to their durable per-run event sinks and heartbeat the runtime lease directly regardless of whether the daemon is alive. Rationale: in-flight agent work is expensive; the supervisor dying must not destroy it. Plain `tusker daemon stop` leaves wrappers running for verified-alive adoption; `tusker daemon stop --drain` waits bounded for them to finish.
