...
**P2 — Dispatch eligibility is one compare-and-swap against task state.** "Should this run?" is answered exactly once, in one shared function, enforced as a conditional update in the store transaction (`WHERE status IN ('ready','rework') AND unleased`), not check-then-act. A task in review/done/backlog/blocked is *structurally* un-dispatchable — the UPDATE touches zero rows. Tusker's eligibility also includes the attempt cap: fresh dispatch, failure retry, continuation retry, reclaim replacement, and redriven retry-queued dispatch all pass through the same pre-attempt guard before any `attempt_count` increment. (Beads `ClaimIssueInTx`; Gas Town redispatch exit-code 3 "not in open status"; opencode's one-run-per-session-key coordinator; OpenHands' atomic `conversation_already_running` check-and-set.)

**P3 — Every loop is bounded, and the bound lands in a terminal parked state with manual redrive.** Separate counters for separate diseases: infra-failure retries (cap + backoff + jitter) and no-progress continuations (small cap, the guard our 88-attempt incident lacked). A process exit code 0 is not proof of progress: if tracker state is still active, classify the attempt as `early_exit` and charge the continuation cap. Reclaim-caused replacements count against the same cap because they spend the same run budget as failed attempts. At the cap: a loud terminal parked state a human must explicitly redrive — never silent, never auto-requeued, and never dispatch first then let the sentinel discover the violation. (SQS `maxReceiveCount`→DLQ; Sidekiq 25→Dead set and super_fetch's 3-crashes-in-72h breaker; Conductor retryCount default 3 hard cap 10; goose `MaxAttemptsReached`; Gas Town max-attempts→escalate-to-Mayor; Aider `max_reflections=3`.)
...
| Per-attempt token budget | **2 M input / 100 k output** | Daemon kills the run on breach (usage events are already ingested; enforcement is a comparison). | OpenHands `max_budget_per_run` checked every step. |
| Per-task token budget | **3 × per-attempt** | Task → parked on breach. This bounds repeated attempts without making normal repair work impossible. | OpenHands; Temporal Schedule-To-Close analogy. |
| Global daily ceiling | **20 M input / 1 M output** | Dispatcher circuit-opens (stops new dispatch, drains running, red banner in serve UI) — never silent overspend. | systemd StartLimit as breaker template; no surveyed system has this — Tusker leads. |
...
Runtime invariant sentinel: at the end of each reconcile tick, the daemon audits the runtime store and opens the invariant circuit with reason `invariant_violation` instead of silently continuing when its own state is impossible. The initial living invariant set is: no execute lease under a non-dispatchable task; no run past retry/continuation caps unless parked or terminal; no fresh heartbeat whose recorded `{pid,start_time}` fails liveness verification; no duplicate active leases for one task and no active lease behind the incumbent lease generation; active-run spend is monotonically non-decreasing across ticks; and `daemon_last_poll_at` advances every tick. Repair stale terminal-record violations with `tusker runs retire <TASK-ID> --reason <why>` followed by `tusker daemon resume`; genuinely live runs still go through `tusker runs interrupt`. Future runtime incidents must add either a sentinel invariant here or an e2e scenario that prevents recurrence.

Terminal states (new, loud, distinct): `parked_no_progress`, `parked_retry_exhausted`, `parked_budget`, `parked_wallclock`. All require explicit `tusker redrive <TASK-ID>` to become dispatchable again. Redrive records who/why, resets the attempt and budget counting windows, clears cooldown/active execution state, and preserves prior attempts, events, evidence, and worktree state. Parked ≠ failed: evidence and worktree are retained, and a redriven task that loops again must park at the same caps before another redrive.
...
6. Poison project registrations must not wedge the daemon. A registered repo/vault whose `WORKFLOW.md` is missing or unloadable is quarantined with `health=error` and retried on later ticks; one broken registration cannot take down polling or serving for healthy projects. → **RUN-T-0021**.
