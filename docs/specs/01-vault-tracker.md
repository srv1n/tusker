...
That does **not** mean the vault stores daemon leases, session heartbeats, retry timers, or in-flight runner state. Those live in the daemon runtime store defined in [03-daemon-and-registry.md](./03-daemon-and-registry.md).
