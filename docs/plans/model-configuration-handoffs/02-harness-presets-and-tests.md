# Complete harness presets, discovery and explicit profile testing
...
Outcome: an operator can select Codex or Muse, discover or manually enter a model, check setup, and explicitly prove that the exact profile can perform a real model turn.

Dependency: agree shared profile/API types with packet 01 first. Then this packet can run alongside 03 and 04. Own catalog/conformance adapter integration and tests; packet 01 owns profile lifecycle and shared endpoint/types, packet 03 owns Settings components. Publish examples so UI work can proceed against stable interfaces. Do not independently edit the same shared API types.

Implementation pointers: inspect cmd/tusker/runner_catalog.go, existing runner conformance handlers, adapter registry and CLI capabilities. Reuse the installed Codex and Muse integration paths and authentication helpers. Verify actual filenames before changes.

Acceptance:
1. Codex and Muse presets bind real existing adapter identities; missing installation produces actionable unavailable status, not fake readiness.
2. Claude Code/OpenCode/Cursor/Devin are unavailable/future unless already implemented and verified; no invented executable flags or remote API assumptions.
3. Discovery is bounded, records source/freshness and preserves manual unknown IDs; reasoning options are model/harness-specific or honest manual entry.
4. Check setup never performs an expensive model turn. Run test requires the explicit action, uses the exact selected profile and reports timeout/cancellation correctly.
5. Catalog success, auth status, and real model execution are independent facts; no secret values enter logs/JSON/UI.
6. CLI and API expose equivalent presets, refresh and test results with revision/profile identity where applicable.
7. Existing conformance invocation remains compatible; no automatic installation, credential retrieval or broad sandbox relaxation.

Verification: extend existing runner catalog/conformance behavioral tests for unsupported discovery, subprocess timeout, manual values, explicit live test invocation and profile identity. Name new focused tests TestModelHarnessPresets and TestModelHarnessProfileTest (or document equivalent existing tests); verify tests actually execute. Coordinate live tests with the resident daemon owner; absence of Muse access is a blocker, not a pass. Update runner/conformance CLI documentation and publish request/response examples to packet 03.
