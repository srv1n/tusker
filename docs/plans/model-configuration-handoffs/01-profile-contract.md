# Complete profile lifecycle and authoritative configuration contracts
...
Verification: extend substantive TestModelLevels and TestServeModelLevels behavioral checks, including removal and snapshot cases. Run go test ./cmd/tusker -run 'Test(ModelLevels|ServeModelLevels)' -count=1 and record nonzero executed tests. Report pre-existing failures separately. Deliver API examples, CLI transcript and owned diff; contract is ready for packet 03/packet 04 only when shared types/actions are stable.
