# Build the Agents profile list and complete add/edit/test flow
...
Depends on story 01/02 published contracts; can run alongside story 04. Own Settings→Agents navigation, Profiles tab/list/editor and retirement of duplicate permission/diagnostic forms. Inspect AppSettings.tsx, app/ProfilesSection.tsx, PermissionsSection.tsx. Story 04 owns the Tiers tab component; agree props/tab mounting first so both do not edit AppSettings concurrently. Shared API/types belong to 02.

## Work

Create a quiet list with a prominent Add profile button. Each row shows readable name, model/effort, compact access summary, tier chips and test state; full fields/references appear only on edit/details. Overflow menu contains disable/enable/remove with reference-aware results. No diagnostic cards or mock future controls below the list.

Add opens one focused form: supported agent; automatically chosen read-only transport if only one, selector if multiple; model and compatible reasoning; supported access options; multi-select tier chips; optional custom display name. Generate a readable default such as Codex · Luna while retaining stable backend IDs. No internal execute-cheap IDs as primary labels. Zero memberships allowed. Explain unavailable integration only when chosen/relevant; do not clutter primary setup with every future adapter.

Show cached discovery immediately and let the non-model metadata refresh run according to 01's policy. Include explicit Refresh models. Loading/error/stale/unsupported states are short and truthful; retain edits and manual exact values. Render only capability-supported option controls using simple existing components. Rare flags go under Advanced; no raw shell flags. Choosing a new model/transport identifies incompatible settings and requires a valid configuration before save; don't silently expand permissions.

Actions: Save, and Save and test. Live test may consume model usage; make this clear adjacent to the action without a large warning panel. Test submits the saved exact profile revision, with running/cancelled/failure/success state; editing marks old proof outdated. It never tests a separately selected diagnostic harness or permission preset. Check setup can be a secondary profile action. Full error/log details remain expandable and redacted.

Remove separate profile permission editing from global Permissions only after migration/02 contract is ready. Preserve independently meaningful project/system restrictions in their correct location. If old controls were nonfunctional placeholders, remove their fake affordances. Do not display unpersisted Full access as current enforced policy.

## Acceptance and verification

- From global Settings, add Codex/Muse profile without memorizing IDs, choose model/effort/access, select two tiers, save/reload and edit without losing values.
- Single/multiple transport, unsupported discovery, manual values, invalid permission combinations, stale writes, failed test and reference-blocked removal work.
- Membership selection does not change primary/fallback assignments silently.
- Narrow and desktop layouts, keyboard/focus and readable control labels pass. No credentials or raw reference lists dominate the page.
- Add real component interaction tests using existing tooling. Run focused bun test paths and bun run typecheck in internal/serve/ui. Record actual file names/commands.
- Capture before/after actual routed Profiles and Add/Edit views under docs/reports/agent-profiles/profiles-ui/. Update relevant Settings documentation; do not call mock rendering live integration.
