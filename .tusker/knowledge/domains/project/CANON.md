...
- Direct interactive work does not require daemon enablement. Its universal work session atomically refuses a healthy automated or interactive owner before the same tracked task is edited, but it is not daemon-dispatch authority and unrelated disabled-daemon state never blocks implementation.
