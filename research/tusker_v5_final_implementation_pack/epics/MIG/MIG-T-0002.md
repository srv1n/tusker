...
id: MIG-T-0002
title: Migrate story files and references to task files
type: task
kind: migration
epic: MIG
status: ready
...
# MIG-T-0002 · Migrate story files and references to task files

## Outcome

Old story files, commands, backlinks, and indexes migrate to task IDs and terminology.

## Acceptance contract

| AC | What must be true | Verification | Deliverables | Doc nodes |
|---:|---|---|---|---|
| 1 | `*-S-*.md` files become `*-T-*.md` with `type: task`. | migration fixture | before/after tree | tusker/migration |
| 2 | Internal wikilinks, docs references, and generated indexes update. | link index validation | links.index output | tusker/migration |
| 3 | Migration report preserves old ID mapping. | report contains mapping | migration report | tusker/migration |
...
- Implement ID map.
- Update backlinks and frontmatter references.
- Emit migration event entries if applying.
...
|---|---|---|---|---|---|---|---|
| changed | work-model | Stories used S IDs and story schema. | Tasks use T IDs and task schema with old ID mapping recorded. | developer, agent | tusker/migration | reference, how-to | pending |
