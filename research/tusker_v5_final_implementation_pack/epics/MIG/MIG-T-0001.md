...
id: MIG-T-0001
title: Build v5 migration dry-run and report
type: task
kind: migration
epic: MIG
status: ready
...
# MIG-T-0001 · Build v5 migration dry-run and report

## Outcome

Users can preview the v5 migration before files are renamed or frontmatter is changed.

## Acceptance contract

| AC | What must be true | Verification | Deliverables | Doc nodes |
|---:|---|---|---|---|
| 1 | `tusker migrate v5 --dry-run` reports renames, field changes, D-note moves, docs-map additions, and warnings. | run on fixture vault | dry-run report | tusker/migration |
| 2 | Dry-run performs no writes. | git diff clean after dry-run | git diff output | tusker/migration |
| 3 | Report is readable by humans and parseable enough for agents. | snapshot test | snapshot output | tusker/migration |

## Scope

### In

- Implement migration planner separate from applier.
- Add old-schema fixtures.
...
|---|---|---|---|---|---|---|---|
| changed | work-model | Migration was hand-wavy and implied manual edits. | Migration has explicit dry-run report agents and humans can inspect. | developer, agent | tusker/migration | reference, how-to | pending |
