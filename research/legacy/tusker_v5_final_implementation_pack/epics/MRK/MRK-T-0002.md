...
id: MRK-T-0002
title: Implement Markdoc node-based rendering first
type: task
kind: feature
epic: MRK
status: ready
...
# MRK-T-0002 · Implement Markdoc node-based rendering first

## Outcome

If Markdoc is enabled, Tusker first uses node customization that keeps documents pure Markdown.

## Acceptance contract

| AC | What must be true | Verification | Deliverables | Doc nodes |
|---:|---|---|---|---|
| 1 | Renderer customizes headings/images/tables without custom tag syntax. | renderer test | test output | tusker/markdoc |
| 2 | Plain Markdown docs still render without Markdoc. | fallback test | test output | tusker/markdoc |
| 3 | Agent-readable Markdown export is unchanged. | snapshot diff | export snapshot | tusker/markdoc |

## Scope

### In

- Add optional site renderer module.
- Keep dependency optional.
- Add feature flag/config gate.
...
|---|---|---|---|---|---|---|---|
| changed | publishing | Publishing had no structured rendering option. | Optional renderer enhances output while source stays plain Markdown. | developer, agent | tusker/markdoc | reference, how-to | pending |
