...
id: MRK-T-0004
title: Add Markdoc validation gate
type: task
kind: feature
epic: MRK
status: ready
...
# MRK-T-0004 · Add Markdoc validation gate

## Outcome

When Markdoc publishing is enabled, docs validate syntax, tag attributes, media references, and forbidden contexts before publishing.

## Acceptance contract

| AC | What must be true | Verification | Deliverables | Doc nodes |
|---:|---|---|---|---|
| 1 | Markdoc syntax validation runs for published docs. | invalid tag fixture | test output | tusker/markdoc |
| 2 | Custom schema validation checks required attributes. | attribute fixture | test output | tusker/markdoc |
| 3 | Forbidden context errors are clear and actionable. | error snapshot | snapshot output | tusker/markdoc |

## Scope

### In

- Add validation command integrated with tusker validate.
- Ensure errors point to file/line when possible.
- Add docs for fixing failures.
...
|---|---|---|---|---|---|---|---|
| changed | publishing | Publishing validation did not enforce structured docs components. | Optional Markdoc docs are validated before publish. | developer, agent | tusker/markdoc | reference, how-to | pending |
