...
id: ARC-T-0001
title: Rename story to task across core model
type: task
kind: migration
epic: ARC
status: ready
...
# ARC-T-0001 · Rename story to task across core model

## Outcome

The repo, CLI, skill, templates, docs, and generated indexes use Task as the executable unit instead of Story.

## Acceptance contract

| AC | What must be true | Verification | Deliverables | Doc nodes |
|---:|---|---|---|---|
| 1 | All user-visible and agent-facing language refers to tasks, except migration notes. | grep/search and review allowed migration references | search output | tusker/model, tusker/tasks |
| 2 | Task IDs use `<EPIC>-T-000N` in templates, fixtures, docs, and indexes. | fixture validation | test output | tusker/model, tusker/tasks |
| 3 | No project layer, story layer, or subtask layer is introduced. | schema and README review | diff summary | tusker/model, tusker/tasks |

## Scope

### In

- Rename commands, templates, docs, examples, and generated index names from story/stories to task/tasks.
- Keep compatibility aliases only for migration.
- Update README data model and working examples.
...
- Update code identifiers carefully.
- Regenerate sample vault after rename.
- Record old-to-new ID mapping for migration tests.
...
|---|---|---|---|---|---|---|---|
| changed | work-model | The executable unit was called Story and used S IDs. | The executable unit is Task and uses T IDs; Story appears only in migration docs. | developer, agent | tusker/model, tusker/tasks | reference, how-to | pending |
