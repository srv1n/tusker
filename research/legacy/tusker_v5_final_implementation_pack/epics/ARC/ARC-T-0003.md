...
id: ARC-T-0003
title: Trim task frontmatter and remove docs enum
type: task
kind: feature
epic: ARC
status: ready
...
Task frontmatter contains only stable routing/current-state fields, and docs work is derived from doc_nodes instead of a docs enum.

## Acceptance contract

| AC | What must be true | Verification | Deliverables | Doc nodes |
|---:|---|---|---|---|
| 1 | Default task frontmatter has only stable routing/current-state fields. | template and fixture review | template diff | tusker/tasks, tusker/docs-system |
| 2 | Runtime, audit, attestation, dispatch, and generated fields are removed from task frontmatter. | validator fixture | validator output | tusker/tasks, tusker/docs-system |
| 3 | The old docs enum is absent; doc_nodes trigger docs impact. | schema/fixture validation | test output | tusker/tasks, tusker/docs-system |

## Scope

### In

- Update schema definitions and templates.
- Move removed fields into events/runs/generated indexes where needed.
- Add migration cleanup for old frontmatter.
...
- Do not hide current task state in events only.
...
|---|---|---|---|---|---|---|---|
| changed | work-model | Task frontmatter carried routing, workflow, audit, dispatch, attestation, docs intent, and generated fields. | Task frontmatter carries readable routing/current-state fields; doc_nodes drive docs impact. | developer, agent | tusker/tasks, tusker/docs-system | reference, how-to | pending |
