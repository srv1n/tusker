...
# EVAL-T-0001 · Define docs eval schema

## Outcome

Tusker has a Markdown/frontmatter schema for docs evals that test retrieval, execution, reference, staleness, media, and promo behavior.

## Acceptance contract

| AC | What must be true | Verification | Deliverables | Doc nodes |
|---:|---|---|---|---|
| 1 | docs eval template exists with kind, doc_nodes, fixture, prompt, expected behavior, must_not, pass criteria, evidence. | template review | template file | tusker/evals |
| 2 | docs evals can be indexed and linked to docs-map nodes. | reindex fixture | evals.index output | tusker/evals |
| 3 | Unknown doc_nodes in evals fail validation. | validator fixture | test output | tusker/evals |

## Scope

### In

- Add schema and template.
- Add parser/indexer.
- Add example evals.
...
|---|---|---|---|---|---|---|---|
| changed | evals | Docs quality was judged by page existence. | Docs quality can be evaluated against task-success scenarios. | developer, agent | tusker/evals | reference, how-to | pending |
