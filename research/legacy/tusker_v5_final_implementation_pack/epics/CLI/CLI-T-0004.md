...
id: CLI-T-0004
title: Update reindex and generated indexes for v5
type: task
kind: feature
epic: CLI
status: ready
...
# CLI-T-0004 · Update reindex and generated indexes for v5

## Outcome

`tusker reindex` rebuilds task, docs, media, eval, links, and dashboard indexes from source files and events.

## Acceptance contract

| AC | What must be true | Verification | Deliverables | Doc nodes |
|---:|---|---|---|---|
| 1 | Deleting `_system/generated` and running reindex reconstructs indexes. | delete/reindex test | test output | tusker/cli |
| 2 | Indexes include tasks, docs, media, evals, links, stale docs, dashboard summaries. | snapshot test | index snapshots | tusker/cli |
| 3 | Reindex does not mutate source files except configured materialized fields. | git diff check | diff output | tusker/cli |
...
- Extend indexer.
- Use docs-map/media-map/events as inputs.
- Add snapshots.
...
|---|---|---|---|---|---|---|---|
| changed | cli | Generated indexes were story/doc/bug oriented. | Generated indexes reflect v5 tasks/docs/media/evals. | developer, agent | tusker/cli | reference, how-to | pending |
