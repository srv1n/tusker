...
func TestEventLogMetadataAcceptsDeviceRenumberingWithStableInodes(t *testing.T) {
	...
	metadata, established, err := readEventLogSequenceMetadataAt(parentFD, base, path)
	if err != nil || !established {
		t.Fatalf("read established sequence metadata: established=%v err=%v", established, err)
	}
	metadata.Device++
	metadata.LockDevice++
	if err := writeEventLogSequenceMetadataAt(parentFD, base, path, metadata, nil); err != nil {
		t.Fatal(err)
	}
	...
	refreshed, established, err := readEventLogSequenceMetadataAt(parentFD, base, path)
	if err != nil || !established {
		t.Fatalf("read refreshed sequence metadata: established=%v err=%v", established, err)
	}
	eventSnapshot, err := snapshotEventLogPathAt(parentFD, base, path)
	if err != nil {
		t.Fatal(err)
	}
	lockSnapshot, err := snapshotEventLogPathAt(parentFD, base+".lock", path+".lock")
	if err != nil {
		t.Fatal(err)
	}
