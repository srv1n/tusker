...
func TestInteractiveWorkReadinessOnlyHonorsGenuineHumanGates(t *testing.T) {
	...
	if blockers := workSessionAdmissionBlockersForLane(task, idx, nil, nil, runLaneExecute); len(blockers) != 0 {
	...
	blockers := workSessionAdmissionBlockersForLane(task, idx, nil, nil, runLaneExecute)
