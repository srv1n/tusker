...
func installedCapabilityCommands() []capabilityCommand {
	return []capabilityCommand{
		...
		{Command: "message", Subcommands: []string{"apply", "ask", "consume", "inbox", "list", "reply", "send", "show"}, Flags: []string{"--body", "--id", "--json", "--key", "--project", "--recipient", "--recipient-kind", "--reply-to", "--sender", "--yield"}, Purpose: "Persist and inspect correlated task or execution messages; transport and wakeup remain capability-gated."},
