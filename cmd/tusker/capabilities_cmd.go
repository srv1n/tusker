...
func installedCapabilityCommands() []capabilityCommand {
	return []capabilityCommand{
		...
		{Command: "delivery", Subcommands: []string{"context", "doctor", "import", "plan", "review", "rollout", "start"}, Flags: []string{"--by", "--confirm", "--json", "--plan", "--scope"}},
		...
		{Command: "models", Subcommands: []string{"catalog", "profile-set", "reset", "set", "show"}, Flags: []string{"--command", "--compact", "--effort", "--harness", "--if-revision", "--json", "--lane", "--level", "--model", "--name", "--preset", "--profiles", "--scope"}},
