...
func (s *RuntimeStore) Migrate() error {
	...
	if err := s.ensureColumn("runs", "reason_code", `ALTER TABLE runs ADD COLUMN reason_code TEXT NOT NULL DEFAULT ''`); err != nil {
		return err
	}
	...
	if err := s.ensureColumn("attempts", "reason_code", `ALTER TABLE attempts ADD COLUMN reason_code TEXT NOT NULL DEFAULT ''`); err != nil {
		return err
	}
...
func (s *RuntimeStore) UpsertRun(run RunStatus) error {
	if run.ReasonCode != "" {
		if _, ok := runFailureReason(RunFailureReasonCode(run.ReasonCode)); !ok {
			return tuskerError(errorInvalidArg, "unknown run failure reason code: "+run.ReasonCode)
		}
	}
	if run.LeaseState == "" {
	...
	_, err := s.exec(`INSERT INTO runs (
		project_id, record_id, item_id, runner, runner_profile, runner_harness, runner_model, runner_effort, runner_fallback_reason, worker_policy_fingerprint, execute_policy_fingerprint, lane, lease_state, lease_owner, lease_generation, lease_expires_at, lease_host, attempt_outcome, active_attempt_id, workspace_path, session_ref, cloud_task_id, cloud_status, cloud_environment_id, cloud_attempt_number, pull_request_url, apply_ref, logs_summary, final_summary, process_pid, process_pgid, process_started_at, prompt_path, event_sink_path, raw_log_path, status_path, work_revision, attempt_count, next_retry_at, last_error, reason_code, infrastructure_json, last_event_at, first_event_at, last_heartbeat_at, terminal, started_at, updated_at
	) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
	ON CONFLICT(project_id, record_id) DO UPDATE SET
		...
		run.ProjectID, run.RecordID, run.ItemID, run.Runner, run.RunnerProfile, run.RunnerHarness, run.RunnerModel, run.RunnerEffort, run.RunnerFallbackReason, run.WorkerPolicyFP, run.ExecutePolicyFP, run.Lane, run.LeaseState, run.LeaseOwner, run.LeaseGeneration, run.LeaseExpiresAt, run.LeaseHost, run.AttemptOutcome, run.ActiveAttemptID, run.WorkspacePath, run.SessionRef, run.CloudTaskID, run.CloudStatus, run.CloudEnvironmentID, run.CloudAttemptNumber, run.PullRequestURL, run.ApplyRef, run.LogsSummary, run.FinalSummary, run.ProcessPID, run.ProcessPGID, run.ProcessStartedAt, run.PromptPath, run.EventSinkPath, run.RawLogPath, run.StatusPath, run.WorkRevision, run.AttemptCount, run.NextRetryAt, run.LastError, run.ReasonCode, marshalRunInfrastructure(run.Infrastructure), run.LastEventAt, run.FirstEventAt, run.LastHeartbeatAt, boolToInt(run.Terminal), run.StartedAt, run.UpdatedAt)
...
var runtimeRunMutableColumns = []string{
	...
	"attempt_count", "next_retry_at", "last_error", "reason_code", "infrastructure_json", "last_event_at", "first_event_at",
...
func runtimeRunMutableValues(run RunStatus) []any {
	return []any{
		...
		run.AttemptCount, run.NextRetryAt, run.LastError, run.ReasonCode, marshalRunInfrastructure(run.Infrastructure), run.LastEventAt, run.FirstEventAt,
...
func (s *RuntimeStore) UpsertRunIfSnapshot(expected, updated RunStatus) (bool, error) {
	if updated.ReasonCode != "" {
		if _, ok := runFailureReason(RunFailureReasonCode(updated.ReasonCode)); !ok {
			return false, tuskerError(errorInvalidArg, "unknown run failure reason code: "+updated.ReasonCode)
		}
	}
...
func saveAttempt(executor runtimeAttemptExecutor, attempt RunAttempt) error {
	if attempt.ReasonCode != "" {
		if _, ok := runFailureReason(RunFailureReasonCode(attempt.ReasonCode)); !ok {
			return tuskerError(errorInvalidArg, "unknown run failure reason code: "+attempt.ReasonCode)
		}
	}
	...
	ON CONFLICT(attempt_id) DO UPDATE SET
		...
		cloud_attempt_number=excluded.cloud_attempt_number,
		pull_request_url=excluded.pull_request_url,
		apply_ref=CASE WHEN excluded.apply_ref != '' THEN excluded.apply_ref ELSE attempts.apply_ref END,
		logs_summary=CASE WHEN excluded.logs_summary != '' THEN excluded.logs_summary ELSE attempts.logs_summary END,
		final_summary=CASE WHEN excluded.final_summary != '' THEN excluded.final_summary ELSE attempts.final_summary END,
		end_state_json=CASE WHEN excluded.end_state_json != '' THEN excluded.end_state_json ELSE attempts.end_state_json END,
		process_pid=excluded.process_pid,
