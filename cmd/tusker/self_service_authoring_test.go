...
func TestSelfServiceAuthoring(t *testing.T) {
	...
	t.Run("A4 changed route contract and owner are rechecked and refused", func(t *testing.T) {
		...
		// The refusal leads with the first defect (the drifted root), while the
		// review names every defect with its repair action; both refuse Start.
		if refusal := directWaveStartRefusal(review); refusal == nil {
			t.Fatal("route change did not refuse Start")
		}
		if _, err := directWaveStart(vault, store, "W-0001", "human:sarav"); err == nil {
			t.Fatal("start authorized a changed route after a passing preflight")
		}
		...
		if refusal := directWaveStartRefusal(review); refusal == nil {
			t.Fatal("start did not refuse a changed owner after a passing preflight")
		}
