...
func TestDaemonTaskDirectiveAuthorizesExactTaskOutsideArmedWave(t *testing.T) {
	...
	if err := writeText(filepath.Join(filepath.Dir(vault), ".gitignore"), "bin/\nfake-codex-args.log\n"); err != nil {
		t.Fatal(err)
	}
	initializeOrchestrationGitRepo(t, filepath.Dir(vault))
	runGitDir(t, filepath.Dir(vault), "add", ".gitignore")
	runGitDir(t, filepath.Dir(vault), "commit", "-m", "ignore fake runner artifacts")
	installFakeCodexExec(t, filepath.Dir(vault))
	project := registerAutomationTestProject(t, vault)
	daemon, err := NewDaemon(DefaultStateRoot())
	if err != nil {
	...
	if blocker, err := daemon.scopeDispatchBlocker(project, task, wf.Data, map[string]RunStatus{}); err != nil || blocker != "" {
		t.Fatalf("exact task directive did not bypass disarmed-wave scope: blocker=%q err=%v", blocker, err)
	}
	if status := stringField(task.Data, "status"); !containsString(wf.Data.Tracker.ActiveStates, status) {
		t.Fatalf("test task is not active: status=%q active=%v", status, wf.Data.Tracker.ActiveStates)
	}
	if err := daemon.PollOnce(context.Background()); err != nil {
		t.Fatal(err)
	}
	...
	if run.AttemptCount != 1 {
		directive, _ := daemon.store.RunDirective(project.ProjectID, "APP-T-0001")
		auth, _ := daemon.store.LatestRunAuthorization(project.ProjectID, "APP-T-0001")
		t.Fatalf("expected exact task directive attempt, got run=%#v directive=%#v auth=%#v", run, directive, auth)
	}
	if run.ActiveAttemptID == "" {
		t.Fatalf("expected task directive to claim one attempt, got %#v", run)
	}
	if run.ProcessPGID > 0 {
		t.Cleanup(func() { _ = syscall.Kill(-run.ProcessPGID, syscall.SIGKILL) })
	}
	...
	if directive == nil || directive.State != "consumed" {
		t.Fatalf("task directive was not consumed, got %#v", directive)
	}
	attempts, err := daemon.store.ListAttemptsForRun(project.ProjectID, "APP-T-0001")
	if err != nil || len(attempts) != 1 {
		t.Fatalf("load directed execute attempt: attempts=%#v err=%v", attempts, err)
	}
	attempts[0].Outcome = string(AttemptOutcomeSucceeded)
	attempts[0].WorkRevision = 1
	if err := daemon.store.SaveAttempt(attempts[0]); err != nil {
		t.Fatal(err)
	}
	run.WorkRevision = 1
