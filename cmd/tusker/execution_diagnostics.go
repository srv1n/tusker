...
func NewDiagnosis(input DiagnosisInput) (Diagnosis, error) {
	...
	for index := range diagnosis.Findings {
		if diagnosis.Findings[index].Evidence.Stale && validDiagnosticClassification(diagnosis.Findings[index].Classification) {
			diagnosis.Findings[index].Classification = DiagnosticUnavailable
		}
	}
