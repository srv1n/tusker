...
func TestSelfServiceAdmission(t *testing.T) {
	t.Run("A1 queued root reservation reaches claim without self exclusion", func(t *testing.T) {
		...
		projected, _, ok, err := armedWaveDispatchTaskProjection(vault, idx.Tasks["APP-T-0001"])
		if err != nil {
			t.Fatalf("dispatch projection errored: %v", err)
		}
		if !ok {
			t.Fatal("dispatch projection refused the armed queued root")
		}
		if status := stringField(projected.Data, "status"); status != "ready" {
			t.Fatalf("dispatch projection left the queued root at status %q", status)
		}
		...
		rootFacts := AdmissionFacts{
			TaskID: "APP-T-0001", Status: stringField(projected.Data, "status"),
			Readiness: stringField(projected.Data, "readiness"), NextOwner: stringField(projected.Data, "next_owner"),
		...
		if verdict := EvaluateAdmissionForStage(dependentFacts, AdmissionStageDaemonDispatch); verdict.Admit {
			t.Fatal("shared admission admitted the dependent before root acceptance")
		} else if verdict.Blockers[0].Code != AdmissionBlockerDependencyWaiting {
			t.Fatalf("dependent refusal blamed the wrong cause: %#v", verdict.Blockers)
		}
	...
	t.Run("A2 stages and lanes agree on code and actor", func(t *testing.T) {
		...
		reviewFacts := AdmissionFacts{
			TaskID: "APP-T-0001", Status: "review", Lane: runLaneReview,
			ContractValid: true, RouteOK: true, ProofMapped: true,
			OwnerFree: true, DependenciesSatisfied: true,
			ProjectRegistered: true, ProjectEnabled: true,
