...
type Workflow struct {
	...
	ModelLevels          map[string]ModelLevelDefinition    `yaml:"model_levels,omitempty" json:"model_levels,omitempty"`
...
For changes_requested, append ` + "`" + reviewerFindingFlagExample("{{ review.material_fingerprint }}") + "`" + `. Keep every blocking and advisory record in that one array. A blocking record requires stable id, acceptance, evidence, consequence, closure_condition, repair_scope, and the injected material fingerprint. Set repair_scope to material when source/implementation/artifact repair is required; set it to proof when only the verification or task ledger must be rerun. Omitted or unknown scope is treated as material.

For a later pass that independently verifies repaired findings, append ` + "`" + reviewerClosureFlagExample("{{ review.material_fingerprint }}") + "`" + `. Copy each prior blocking finding id and closure_condition exactly, add current evidence, and bind the exact current material fingerprint. The later review attempt must be independent. A material-scoped finding requires changed material; a proof-scoped finding may close on the same material after its proof/task ledger is refreshed. Never create irrelevant source churn just to close a proof-scoped finding.
...
func applyTuskerAutomationConfig(vaultPath string, wfFile WorkflowFile) (WorkflowFile, error) {
	...
	wf.ModelLevels = modelLevelsFromSchema(cfg.Automation.ModelLevels)
