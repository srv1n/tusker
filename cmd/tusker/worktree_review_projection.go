...
func projectCompletedWorktreeReviewToCanonical(canonicalVault string, run RunStatus) (string, error) {
	...
	if taskID == "" || strings.TrimSpace(run.ActiveAttemptID) == "" || strings.TrimSpace(run.WorkspacePath) == "" {
		return "", tuskerError(errorInvalidTransition, "execution review projection requires task, active attempt, and workspace identity")
	}
	if run.Lane != runLaneExecute {
		return "", tuskerError(errorInvalidTransition, "execution review projection requires an execute-lane run")
	}
	...
	if worktreeVault == "" || workspacePathsCompatible(worktreeVault, canonicalVault) {
		return "", tuskerError(errorInvalidTransition, "execution review projection requires an isolated worktree vault")
	}
	...
	if !ok {
		return "", tuskerError(errorNotFound, "execution review projection cannot find worktree task "+taskID)
	}
	var bound []Note
	...
	if len(bound) != 1 {
		return "", tuskerError("CAS_CONFLICT", "execution review projection requires exactly one worktree attempt bound to runtime attempt "+run.ActiveAttemptID)
	}
	attempt := bound[0]
	if stringField(attempt.Data, "status") != "handoff" ||
		...
		return "", tuskerError(errorInvalidTransition, "execution review projection rejected mismatched worktree attempt binding")
	}
	startRevision := strings.TrimSpace(stringField(attempt.Data, "task_state_rev"))
	if startRevision == "" {
		return "", tuskerError(errorInvalidTransition, "execution review projection requires the attempt's immutable task-state revision")
	}
	if stringField(localTask.Data, "status") != "review" || stringField(localTask.Data, "proof_status") != "satisfied" {
		return "", tuskerError(errorInvalidTransition, "execution review projection requires a proof-satisfied worktree task in review")
	}
	branch, err := currentGitBranchIn(run.WorkspacePath)
	if err != nil || !strings.EqualFold(branch, v7TaskBranchName(taskID)) {
		return "", tuskerError(errorInvalidTransition, "execution review projection requires checked-out implementation branch "+v7TaskBranchName(taskID))
	}
	sourceSHA, ok := gitRevParse(run.WorkspacePath, "HEAD^{commit}")
	if !ok {
		return "", tuskerError(errorInvalidTransition, "execution review projection requires a committed implementation source")
	}
	...
	if canonicalRevision != startRevision {
		return "", tuskerError("CAS_CONFLICT", "execution review projection refused because canonical task changed after dispatch")
	}
	...
	if !v7StateRevMatches(data, body, stringField(data, "state_rev")) {
		return "", tuskerError(errorInvalidTransition, "execution review projection rejected malformed worktree task state")
	}
