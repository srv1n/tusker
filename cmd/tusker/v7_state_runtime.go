...
func buildV7Dashboards(vaultPath string, idx v7Index) error {
	...
	if err := writeJSON(filepath.Join(vaultPath, "_generated", "indexes", "dashboard.json"), map[string]any{
...
func v7CommittedDashboardProjections(idx v7Index, leases []v7LeaseRecord) map[string]string {
	return map[string]string{
		...
		"_generated/bases/agent-ready.base":   "filters:\n  readiness: ready\n  status: [ready, rework]\n  next_owner_prefix: agent\n",
		"_generated/bases/human-actions.base": "filters:\n  kind: gate\n  status: open\n  owner_prefix: human:\n",
	}
}
