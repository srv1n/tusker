...
func departureGateIntent(wf Workflow, repoRoot, treeHash string) DepartureGate {
	...
	return DepartureGate{Command: command, Profile: firstNonEmpty(wf.Orchestration.Gate.Profile, wf.Orchestration.BatchGate.FeatureProfile), Toolchain: scheduledPromotionFullGateToolchainFingerprint(repoRoot, commands, wf.Orchestration.Gate.IsolationProvider, DefaultStateRoot()), TreeHash: treeHash, Status: "required"}
}
