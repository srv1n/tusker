...
func deliveryDoctorContractFinding(issue string) (string, string, []string, string) {
	taskKey := ""
	if i := strings.Index(issue, ":"); i > 0 {
		taskKey = strings.TrimSpace(issue[:i])
	}
	...
	case strings.Contains(issue, "acceptance ") && strings.Contains(issue, " has no mapped verification"):
		return "ACCEPTANCE_UNMAPPED", taskPath("acceptance"), keys, "add an exact verification row covering the acceptance ID"
	case strings.Contains(issue, "verification references unknown acceptance"):
		return "PROOF_ACCEPTANCE_UNKNOWN", taskPath("verification"), keys, "map verification only to declared acceptance IDs"
	case strings.Contains(issue, "verification must use an exact"):
		return "PROOF_UNSUPPORTED", taskPath("verification"), keys, "use an exact supported command: or manual proof: check"
	case strings.Contains(issue, "verification is required"):
		return "PROOF_UNFILLED", taskPath("verification"), keys, "add exact verification covering every acceptance ID"
	case strings.Contains(issue, "acceptance rows require") || strings.Contains(issue, "duplicate acceptance id"):
		return "ACCEPTANCE_INVALID", taskPath("acceptance"), keys, "give every acceptance row a unique stable ID and concrete outcome"
	case strings.Contains(issue, "artifact"):
		return "ARTIFACT_INVALID", taskPath("artifact"), keys, "declare a valid operator-facing artifact with covered acceptance IDs"
	case strings.Contains(issue, "cycle"):
		return "DEPENDENCY_CYCLE", "tasks.*.dependencies", nil, "remove or redirect dependency edges until the graph is acyclic"
	case strings.Contains(issue, "dangling dependency"):
		return "DEPENDENCY_DANGLING", taskPath("dependencies"), keys, "reference an existing task source_key"
	case strings.Contains(issue, "unknown requirement"):
		return "REQUIREMENT_REFERENCE_UNKNOWN", taskPath("requirement_refs"), keys, "reference a declared requirement ID"
	case strings.Contains(issue, "runner"):
		return "UNSUPPORTED_RUNNER", taskPath("runner_profile"), keys, "choose a configured runner profile"
	default:
